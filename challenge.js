...
function fetchData(urlApi, callback) {
    ...
    //El metodo .open realiza la petición de apertura de comunicación, el metodo puede ser 'GET' o 'POST', luego se envia la URL, 
    //si es asincrono (true o false), usuario y contraseña. En esta caso solo se utiliza el metodo, la url y async
    ...
    xhttp.onreadystatechange = function (event) {
        ...
        if (xhttp.readyState === 4) {
            ...
            if (xhttp.status === 200) {
                //se ejecuta el callback recibiendo como argumentos un objeto, como la respuesta de la API es un texto plano, 
                //el metodo JSON.parse tranformará este texto en un objeto.
                //El atributo devuelve un DOMString que contiene la  respuesta a la consulta como un texto o null 
                //si la consulta no tuvo exito o aun no ha sido completada.
...
fetchData(`${API}/products`, function (error1, data1) {
    ...
    //se envia como parámetros la url de la API apuntando al atributo del primer objeto de arreglo data1 y nuevamente una función anónima.
